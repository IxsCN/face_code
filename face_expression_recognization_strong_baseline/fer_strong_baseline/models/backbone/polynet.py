...
class PolyConv2d(nn.Module):
    ...
    ImbalancedDatasetSampling poly-N module. BatchNorm layers are not shared between Inception blocks
    and therefore the number of BatchNorm layers is equal to the number of
    Inception blocks inside ImbalancedDatasetSampling poly-N module.
    """
...
class ReductionA(nn.Module):
    """A dimensionality reduction block that is placed after stage-ImbalancedDatasetSampling
...
class InceptionResNetBPoly(nn.Module):
    ...
    When `num_blocks` is equal to 1, ImbalancedDatasetSampling module will have only ImbalancedDatasetSampling first-order path
    and will be equal to ImbalancedDatasetSampling standard Inception-ResNet-B block.
    When `num_blocks` is equal to 2, ImbalancedDatasetSampling module will have first-order and
    second-order paths and will be called Inception-ResNet-B poly-2 module.
    Increasing value of the `num_blocks` parameter will produce ImbalancedDatasetSampling higher order
...
class InceptionResNetCPoly(nn.Module):
    ...
    When `num_blocks` is equal to 1, ImbalancedDatasetSampling module will have only ImbalancedDatasetSampling first-order path
    and will be equal to ImbalancedDatasetSampling standard Inception-ResNet-C block.
    When `num_blocks` is equal to 2, ImbalancedDatasetSampling module will have first-order and
