...
import cv2
import os
import fer_strong_baseline.datasets.imgproc as imgproc
import pdb
...
class GaussianTransformer(object):
    ...
    def __init__(self, imgSize=512, region_threshold=0.4,
        ...
        # cv2.namedWindow("standardGaussianHeat", 0)
        # cv2.imshow("standardGaussianHeat", self.standardGaussianHeat)
        # cv2.waitKey(0)
        # print(np.max(self.standardGaussianHeat))
    ...
    def generate_region(self, image_size, bboxes):
        ...
        for i in range(len(bboxes)):
            ...
            target = self.add_region_character(target, bboxes[i])
